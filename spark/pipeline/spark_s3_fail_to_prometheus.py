...
# --- 설정 ---
PROMETHEUS_PORT = 9992  # 다른 스크립트와 충돌하지 않는 새 포트
...
FAIL_METRICS = {
    'sensor1': Gauge('fms_fail_latest_sensor1', 'Latest fail value of sensor1', METRIC_LABELS),
    'sensor2': Gauge('fms_fail_latest_sensor2', 'Latest fail value of sensor2', METRIC_LABELS),
    'sensor3': Gauge('fms_fail_latest_sensor3', 'Latest fail value of sensor3', METRIC_LABELS),
    'motor1': Gauge('fms_fail_latest_motor1', 'Latest fail value of motor1', METRIC_LABELS),
    'motor2': Gauge('fms_fail_latest_motor2', 'Latest fail value of motor2', METRIC_LABELS),
    'motor3': Gauge('fms_fail_latest_motor3', 'Latest fail value of motor3', METRIC_LABELS),
}
...
def update_fail_metrics(spark):
    ...
    try:
        # 1. S3 경로의 JSON 파일 읽기
        # 1. 파티셔닝된 경로만 읽기
        df_fail = spark.read.text(partition_path)
        
        # 2. JSON 파싱 및 컬럼 추출
        df_parsed = df_fail.select(from_json(col("value"), JSON_SCHEMA).alias("data")).select("data.*")
        ...
        if df_recent.rdd.isEmpty():
            print("✅ 처리할 최신 'fail' 데이터가 없습니다.")
            for gauge in FAIL_METRICS.values():
                gauge.clear()
            return
        ...
        for row in df_latest.collect():
            ...
            for metric_name, gauge in FAIL_METRICS.items():
                value = row[metric_name]
                if value is not None:
                    gauge.labels(**labels).set(float(value))
        
        print(f"✅ {df_latest.count()}개 'fail' 장비의 최신 메트릭을 성공적으로 업데이트했습니다.")

    except Exception as e:
        if "Path does not exist" in str(e):
            print(f"⚠️ S3 경로를 찾을 수 없습니다: {partition_path}. 'fail' 데이터가 아직 없을 수 있습니다.")
        else:
...
def main():
    ...
    spark = SparkSession.builder \
        .appName("FMS S3 Fail to Prometheus") \
    ...
    print(f"🚀 'fail' 데이터 프로메테우스 메트릭 서버를 시작합니다. (포트: {PROMETHEUS_PORT})")
    start_http_server(PROMETHEUS_PORT)
    print(f"🔗 메트릭 확인: http://<your-spark-driver-ip>:{PROMETHEUS_PORT}")

    try:
        while True:
            update_fail_metrics(spark)
